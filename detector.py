...
def capture_faces(name: str, save_dir: Path = Path("training"), max_images=10):
    ...
    while cap.isOpened() and count < max_images:
        ...
        if key == ord("s"):
            filename = f"{name}_{count+1:02d}.jpg"
...
def recognize_from_webcam_simple(encodings_location: Path = DEFAULT_ENCODINGS_PATH, model: str = "hog", tolerance: float = 0.6):
    ...
    while cap.isOpened():
        ...
        cv2.imshow("Webcam Face Recognition (Simple)", frame)
...
if __name__ == "__main__":
    ...
    if args.test:
        if not args.f:
            print("Vui lòng cung cấp đường dẫn ảnh với -f")
        else:
            recognition_unknown_face(image_location=args.f, model=args.m)
    if args.realtime:
        recognize_from_webcam_simple(encodings_location=DEFAULT_ENCODINGS_PATH, model=args.m)
