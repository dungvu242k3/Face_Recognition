"""def recognize_from_webcam_yolo(encodings_location: Path = DEFAULT_ENCODINGS_PATH, tolerance: float = 0.6):
    ...
    model = YOLO("yolov8n.pt")  # hoặc model YOLO face detection nếu có
    ...
    while cap.isOpened():
        ...
        for box in results.boxes:
            ...
            # Dùng face_recognition để tìm chính xác box mặt trong vùng crop này
            face_locations = face_recognition.face_locations(rgb_cropped)

            for (top, right, bottom, left) in face_locations:
                # Lấy encoding khuôn mặt nhỏ này
                ...
                # Vẽ box chính xác khuôn mặt lên frame gốc (cần dịch tọa độ về frame lớn)
                abs_left = x1 + left
                abs_top = y1 + top
                abs_right = x1 + right
